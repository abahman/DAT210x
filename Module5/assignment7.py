...
#
import pandas as pd
header_title = ['sample', 'thickness', 'size', 'shape', 'adhesion', 'epithelial', 'nuclei', 'chromatin', 'nucleoli', 'mitoses', 'status']
X = pd.read_csv('/Users/ammarbahman/desktop/dat210x/module5/Datasets/breast-cancer-wisconsin.data',sep =',',names=header_title)

#print X.dtypes
X['nuclei'] = pd.to_numeric(X['nuclei'], errors='coerce') #covert 'nuclei' from object to int, this casue some values to convert to NaNs
...
#
y.fillna(y.mean()) #y is the label series
...
model = None
if Test_PCA:
  print "Computing 2D Principle Components (PCA)"
  #
  ...
  print "Computing 2D Isomap Manifold (Isomap)"
  #
  ...
  iso = manifold.Isomap(n_neighbors=5, n_components=2) #n_neighbors is K value
  iso.fit(T)
...
#
# .. your code here .. This alraedy been done .transform(T)
X = model
...
model = KNeighborsClassifier(n_neighbors=13,weights='distance') #n_neighbors is KNeighbors values. weights={'uniform','distance'}
...
#print accuracy_score(y_train, predictions, normalize=True)
